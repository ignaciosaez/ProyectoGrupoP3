package COMUN;

public class constantesActionCommand 
{
	/**
	 * Constantes para los ActionCommand de los elementos del interfaz de usuario.
	 */
	private static final long serialVersionUID = 1L;
	
	
	public final static String BUTTON_ENTRAR = "BUTTON_ENTRAR";
	public final static String BUTTON_CANCELAR = "BUTTON_CANCELAR";
	public final static String BUTTON_REGISTARSE = "BUTTON_REGISTRASE";
	public final static String BUTTON_ANADIRPRODUCTO= " BUTTON_ANADIRPRODUCTO";
	public final static String BUTTON_VERPRODUCTOS= " BUTTON_VERPRODUCTOS";
	public final static String BUTTON_VERPRODUCTOSACTUALIZADOS= " BUTTON_VERPRODUCTOSACTUALIZADOS";
	public final static String BUTTON_MODIFICARPRODUCTOS= " BUTTON_MODIFICARPRODUCTOS";
	public final static String BUTTON_COMPRARPRODUCTOS= " BUTTON_COMPRARPRODUCTOS";
	public final static String BUTTON_ELIMINARDELCARRITO= "BUTTON_ELIMINARDELCARRITO";
	public final static String BUTTON_ANADIRCARRITO= "BUTTON_ANADIRCARRITO";
}
